...
const Accordion = ({ service, index }: CoreFeatureProps) => {
    ...
    <div className="accordion-item">
      <h2 className="accordion-header" id="headingOne">
        <button onClick={() => toggle()} style={btnStyle} aria-expanded="true" aria-controls="collapse">
          {title} <span style={{transform: open ? "rotate(-180deg)" : "rotate(0)", transition: "all .3s ease"}}><FaChevronDown/></span>
        </button>
      </h2>
